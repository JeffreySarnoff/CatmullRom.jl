struct CubicPoly{T}
    ...
    #=
        Compute coefficients for a cubic polynomial
	        cubicpoly(s) = c0 + c1*s + c2*s^2 + c3*s^3
	      such that
	        cubicpoly(0) = coord0, cubicpoly(1) = coord1
	      and
	        d1cubicpoly(0) = deriv0, d1cubicpoly(1) = deriv1
    =#
    function CubicPoly(coord0::T, coord1::T, deriv0::T, deriv1::T) where {T}
        c0 = coord0
        c1 = deriv0
        c2 = -3 * coord0 + 3 * coord1 - 2 * deriv0 - deriv1
        c3 =  2 * coord0 - 2 * coord1 +     deriv0 + deriv1
        return new{T}(c0, c1, c2, c3)
    end
    
    function CubicPoly(cs::NTuple{4,T}) where T
        return new{T}(cs...)
    end
end
...
#=
/* https://raw.githubusercontent.com/mrdoob/three.js/master/src/extras/curves/CatmullRomCurve3.js */
